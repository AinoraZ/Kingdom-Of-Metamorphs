...
public class TurnHandler : MonoBehaviour {
	...
	void Update() {
		if (firstTime) {
			...
			else if (GameObject.FindGameObjectWithTag("P1").GetComponent<Minion>().moved && Move) {
	...
	public void EndTurn() {
		...
		for (int x = 0; x < GameObject.FindGameObjectsWithTag("P1").Length; x++) {
			GameObject.FindGameObjectsWithTag("P1")[x].GetComponent<Minion>().moved = false;
			GameObject.FindGameObjectsWithTag("P1")[x].GetComponent<MinionInfo>().Switch();
		}
		for (int x = 0; x < GameObject.FindGameObjectsWithTag("P2").Length; x++) {
			GameObject.FindGameObjectsWithTag("P2")[x].GetComponent<MinionInfo>().Switch();
		}
